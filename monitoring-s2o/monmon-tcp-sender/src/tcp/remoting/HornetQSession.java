...
import es.s2o.monmon.tcp.core.SimpleString;

/**
 * HornetQ implementation of a JMS Session. <br>
...
public class HornetQSession implements QueueSession, TopicSession {
	...
	protected HornetQSession(final HornetQConnection connection, final boolean transacted, final boolean xa,
	...
	public TopicSubscriber createDurableSubscriber(final Topic topic, final String name, String messageSelector,
			final boolean noLocal) throws JMSException {
		// As per spec. section 4.11
		if (sessionType == HornetQSession.TYPE_QUEUE_SESSION) {
	...
	private HornetQMessageConsumer createConsumer(final HornetQDestination dest, final String subscriptionName,
			String selectorString, final boolean noLocal) throws JMSException {
		try {
			...
			if (noLocal) {
				...
				if (connection.getClientID() != null) {
					filter = HornetQConnection.CONNECTION_ID_PROPERTY_NAME.toString() + "<>'"
			...
			if (dest.isQueue()) {
				...
				if (subscriptionName == null) {
					...
					queueName = new SimpleString(HornetQDestination.createQueueNameForDurableSubscription(
					...
					if (!subResponse.isExists()) {
						...
						boolean selectorChanged = coreFilterString == null && oldFilterString != null
								|| oldFilterString == null && coreFilterString != null || oldFilterString != null
								&& coreFilterString != null && !oldFilterString.equals(coreFilterString);
			...
			HornetQMessageConsumer jbc = new HornetQMessageConsumer(this, consumer, noLocal, dest, selectorString,
					autoDeleteQueueName);
	...
	public QueueBrowser createBrowser(final Queue queue, String filterString) throws JMSException {
		...
		try {
			if (filterString != null) {
				new FilterParser()
						.parse(new SimpleString(filterString.trim()), new HashMap<SimpleString, Identifier>());
			}
		}
		catch (ParseException e) {
			throw JMSExceptionHelper.convertFromHornetQException(HornetQJMSClientBundle.BUNDLE.invalidFilter(e,
	...
	public void unsubscribe(final String name) throws JMSException {
		...
		SimpleString queueName = new SimpleString(HornetQDestination.createQueueNameForDurableSubscription(
				connection.getClientID(), name));
