...
import es.s2o.monmon.tcp.core.FutureLatch;
import es.s2o.monmon.tcp.core.HornetQException;
import es.s2o.monmon.tcp.remoting.AbstractConnector;
import es.s2o.monmon.tcp.remoting.BufferHandler;
import es.s2o.monmon.tcp.remoting.Connection;
import es.s2o.monmon.tcp.remoting.ConnectionLifeCycleListener;
import es.s2o.monmon.tcp.remoting.HornetQComponent;
import es.s2o.monmon.tcp.remoting.ProtocolType;
...
public class NettyConnector extends AbstractConnector {
	...
	private VirtualExecutorService virtualExecutor;
	...
	public NettyConnector(final Map<String, Object> configuration, final BufferHandler handler,
			final ConnectionLifeCycleListener listener, final Executor closeExecutor, final Executor threadPool,
		...
		// httpClientIdleScanPeriod = ConfigurationHelper.getLongProperty(
		...
		// TransportConstants.DEFAULT_USE_NIO_CLIENT, configuration);
		//
		// nioRemotingThreads = ConfigurationHelper.getIntProperty(TransportConstants.NIO_REMOTING_THREADS_PROPNAME, -1,
		// configuration);
		//
		...
		// TransportConstants.DEFAULT_KEYSTORE_PATH, configuration);
		// keyStorePassword = ConfigurationHelper.getPasswordProperty(TransportConstants.KEYSTORE_PASSWORD_PROP_NAME,
		// TransportConstants.DEFAULT_KEYSTORE_PASSWORD, configuration,
		// HornetQDefaultConfiguration.PROP_MASK_PASSWORD, HornetQDefaultConfiguration.PROP_MASK_PASSWORD);
		// }
		// else {
		// keyStorePath = null;
		...
		// TransportConstants.DEFAULT_BATCH_DELAY, configuration);
		//
	...
	public String toString() {
		return "NettyConnector [host=" + host + ", port=" + port + ", httpEnabled=" + httpEnabled + ", useServlet="
				+ useServlet + ", servletPath=" + servletPath + ", sslEnabled=" + sslEnabled + ", useNio=" + useNio
				+ "]";
	}
	...
	public Connection createConnection() {
		...
		if (future.isSuccess()) {
			...
			NettyConnection conn = new NettyConnection(configuration, ch, connectionListener, !httpEnabled
					&& batchDelay > 0, false);
	...
	private static final class HornetQClientChannelHandler extends HornetQChannelHandler {
		HornetQClientChannelHandler(final ChannelGroup group, final BufferHandler handler,
		...
		public void channelConnected(final ChannelHandlerContext ctx, final ChannelStateEvent e) throws Exception {
			...
			if (httpClientIdleScanPeriod > 0) {
				...
				java.util.concurrent.Future<?> future = scheduledThreadPool.scheduleAtFixedRate(task,
						httpClientIdleScanPeriod, httpClientIdleScanPeriod, TimeUnit.MILLISECONDS);
	...
	private class Listener implements ConnectionLifeCycleListener {
		public void connectionCreated(final HornetQComponent component, final Connection connection,
	...
	public boolean isEquivalent(Map<String, Object> configuration) {
		...
		// HornetQClientLogger.LOGGER.debug(this + " host 1: " + host + " ip address: " + ip1 + " host 2: "
		// + this.host + " ip address: " + ip2);
		//
